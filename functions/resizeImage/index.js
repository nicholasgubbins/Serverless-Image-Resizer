...
function gmCreator(asset, bucket, resize_options){ // function to create a GM process
  return new Promise((resolve, reject) => {
    try{
      var func = gm(s3.getObject({Bucket:bucket, Key:asset}).createReadStream());
...
function uploadToS3(destination, bucket, mime_type, storage_class, gm){
  return new Promise((resolve, reject) => {
    try{
      let target_stream = S3Stream.upload({
        Bucket: bucket,
        Key: destination,
        ContentType: mime_type,
        StorageClass: storage_class,
      });
      let file_type = mime_type.substring(mime_type.indexOf('/')+1);
      gm.stream(file_type, function(err, stdout, stderr){
        if (err){
          return reject({statusCode:500, body:`{"message":"${err.message}"}`});
        }
        else{
          ...
          stderr.on('data', function(data){
            return reject({statusCode:500, body:`{"message":"${data}"}`});
          });
          stdout.pipe(target_stream)
          .on('uploaded', function(done){
            return resolve();
          })
...
module.exports.handler = (event, context, callback) => {
  ...
  .then(job => {
    return gmCreator(job.asset, job.bucket, job.resize_options)
    .then(gm => {
      return uploadToS3(job.destination, job.bucket, job.mime_type, job.storage_class, gm)
    })
    .then(() => {
